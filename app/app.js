...
async function saveTelegramSettings() {
  ...
  if (!botToken || !chatId) {
    if (status) { status.style.color = 'var(--error)'; status.textContent = '⚠️ أدخل Bot Token وChat ID أولاً'; }
    return;
  }
  ...
  if (status) { status.style.color = 'var(--success)'; status.textContent = '✅ تم الحفظ'; }
...
async function telegramBackup() {
  ...
  if (status)  { status.style.color = 'var(--text2)'; status.textContent = '⏳ جارٍ الإرسال…'; }
  ...
  try {
    ...
    if (res?.ok) {
      ...
      if (status) { status.style.color = 'var(--success)'; status.textContent = '✅ تم إرسال النسخة الاحتياطية إلى Drive'; }
      _showBackupResultNotif(true, null);
      toast('✅ تم إرسال النسخة الاحتياطية إلى Drive');
    } else {
      // ── Failure ──────────────────────────────────────────
      const msg = res?.error || 'فشل الإرسال';
      if (status) { status.style.color = 'var(--error)'; status.textContent = '❌ ' + msg; }
      _showBackupResultNotif(false, msg);
    }
  } catch(e) {
    if (status) { status.style.color = 'var(--error)'; status.textContent = '❌ ' + e.message; }
...
function _showBackupResultNotif(success, errorMsg) {
  ...
  const title = success ? 'تم إرسال النسخة الاحتياطية ✅' : 'فشل إرسال النسخة الاحتياطية';
