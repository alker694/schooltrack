...
async function saveHoliday() {
  const data = { date:document.getElementById('fHolidayDate').value, type:document.getElementById('fHolidayType').value, reason:document.getElementById('fHolidayReason').value.trim() };
  if (!data.date) return toast('يرجى اختيار تاريخ');
  await apiFetch('/holidays', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data) });
  closeModal('holidayModal'); await loadAll(); renderHolidayList(); toast('✅ تمت إضافة الإجازة');
}
...
async function waLoadLog() {
  const data = await apiFetch('/whatsapp/log');
  _waLogData = data || [];
  waRenderLog();
}
...
//  تبويب: المجموعات
// ══════════════════════════════════════════════════════════════════
function waRenderGroups() {
  const el = document.getElementById('waGroupsList');
  if (!el) return;
  if (_waGroupsData.length === 0) {
    el.innerHTML = `
      <div class="wa-empty">
        <div class="wa-empty-icon">👥</div>
        <div class="wa-empty-title">لا توجد مجموعات</div>
        <div class="wa-empty-sub">أنشئ مجموعة لإرسال رسائل جماعية</div>
      </div>`;
    return;
  }
  el.innerHTML = _waGroupsData.map(g => `
    <div class="wa-group-card">
      <div class="wa-group-card-top">
        <div class="wa-group-icon">👥</div>
        <div class="wa-group-card-info">
          <div class="wa-group-card-name">${g.name}</div>
          <div class="wa-group-card-count">${g.members.length} عضو</div>
        </div>
        <div class="wa-group-card-actions">
          <button class="wa-group-btn wa-group-send"   onclick="waSendToGroup('${g.id}')">📤 إرسال</button>
          <button class="wa-group-btn wa-group-edit"   onclick="waOpenEditGroup('${g.id}')">✏️</button>
          <button class="wa-group-btn wa-group-delete" onclick="waDeleteGroup('${g.id}')">🗑</button>
        </div>
      </div>
      ${g.members.length > 0 ? `
        <div class="wa-group-members-preview">
          ${g.members.slice(0,5).map(m => `
            <span class="wa-group-member-chip" title="${m.phone}">${m.name||m.phone}</span>
          `).join('')}
          ${g.members.length > 5 ? `<span class="wa-group-member-chip wa-more-chip">+${g.members.length-5}</span>` : ''}
        </div>
      ` : ''}
    </div>
  `).join('');
...
function waOpenNewGroup() {
  _editingGroupId = null;
  document.getElementById('waGroupModalTitle').textContent = 'مجموعة جديدة';
  document.getElementById('waGroupName').value = '';
  document.getElementById('waGroupMembersList').innerHTML = '';
  document.getElementById('waImportClassSel').classList.add('hidden');
  closeModal('waGroupModal'); // reset
  openModal('waGroupModal');
  waAddMemberRow(); // start with one empty row
}

function waOpenEditGroup(id) {
  const g = _waGroupsData.find(x => x.id === id);
  if (!g) return;
  _editingGroupId = id;
  document.getElementById('waGroupModalTitle').textContent = `تعديل: ${g.name}`;
  document.getElementById('waGroupName').value = g.name;
  document.getElementById('waGroupMembersList').innerHTML = '';
  document.getElementById('waImportClassSel').classList.add('hidden');
  openModal('waGroupModal');
  g.members.forEach(() => {});  // re-render
  const list = document.getElementById('waGroupMembersList');
  list.innerHTML = g.members.map((m, i) => waMemberRowHTML(i, m.name, m.phone)).join('');
}
...
async function waSaveGroup() {
  ...
  if (_editingGroupId) {
    await apiFetch(`/whatsapp/groups/${_editingGroupId}`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
...
function waRenderLog() {
  ...
  let log = _waLogData;
  if (filter === 'absence') log = log.filter(l => l.type === 'absence');
  if (filter === 'custom')  log = log.filter(l => l.type === 'custom');
  if (filter === 'sent')    log = log.filter(l => l.status === 'sent');
  if (filter === 'failed')  log = log.filter(l => l.status === 'failed');
  ...
  if (log.length === 0) {
    el.innerHTML = `
      <div class="wa-empty">
        ...
        <div class="wa-empty-sub">ستظهر هنا كل الرسائل المرسلة</div>
      </div>`;
    return;
  }
  ...
  el.innerHTML = log.map(entry => {
    ...
    return `
      <div class="wa-log-item ${entry.status==='sent'?'wa-log-sent':'wa-log-failed'}">
        <div class="wa-log-item-left">
          <div class="wa-log-status-dot ${entry.status==='sent'?'wa-dot-sent':'wa-dot-failed'}"></div>
          <div class="wa-log-content">
            <div class="wa-log-recipient">
              ${isAbsence ? `<span class="wa-log-type-badge wa-badge-absence">غياب</span>` : `<span class="wa-log-type-badge wa-badge-custom">مخصص</span>`}
              ${entry.studentName || entry.phone}
            </div>
            <div class="wa-log-meta">
              ${entry.phone} · ${timeStr}
              ${isAbsence && entry.className ? ` · ${entry.className}` : ''}
            </div>
...
function calOpenDay(iso) {
  ...
  const dayEvs=_calAllEvents.filter(e=>e.date===iso||(e.endDate&&iso>=e.date&&iso<=e.endDate));
...
function calSelectType(type){
  ...
  document.querySelectorAll('.cal-color-btn').forEach(b=>b.classList.toggle('active',b.dataset.color===_calSelColor));
  document.getElementById('calWaSection')?.classList.toggle('hidden',type!=='message');
  document.getElementById('calSpecificDaysSection')?.classList.toggle('hidden',type!=='message');
...
/* WhatsApp recipients */
function calWaAddAllParents(){
  const all=(state.students||[]).filter(s=>s.parentPhone);
  all.forEach(s=>{ if(!_calWaRows.find(r=>r.phone===s.parentPhone)) _calWaRows.push({name:s.name,phone:s.parentPhone}); });
  _renderCalWaRows();
  toast(`✅ تم إضافة ${all.length} ولي أمر`);
}
function calWaAddGroupChat(){
  _calWaRows.push({ name: 'مجموعة واتساب', phone: '' , isGroup: true });
  _renderCalWaRows();
}
...
function _renderCalWaRows(){
  ...
  el.innerHTML=_calWaRows.map((r,i)=>`
    <div class="cal-wa-row">
      <input type="text" class="cal-wa-name"  placeholder="${r.isGroup?'اسم المجموعة':'الاسم'}" value="${r.name||''}" onchange="_calWaRows[${i}].name=this.value" />
      <input type="text" class="cal-wa-phone" placeholder="${r.isGroup?'Group Chat ID (مثال: 120363...@g.us)':'رقم الواتساب'}" value="${r.phone||''}" dir="ltr" onchange="_calWaRows[${i}].phone=this.value" />
...
async function saveCalEvent(){
  ...
  const payload={
    ...
    color:         _calSelColor,
    dailyRepeat,
    specificDates: useSpecific ? _specificDates : null,
