...
async function fonnteLoadQR() {
  ...
  try {
    ...
    // Fonnte returns { status:true, url:"iVBORw0KGgo..." } where url is base64 PNG data
    var qrBase64 = data.url || data.base64 || data.qr || data.image || data.qrcode || null;

    if (qrBase64 && qrBase64.length > 100) {
      if (spinner) spinner.classList.add('hidden');
      if (imgEl) {
        imgEl.src = 'data:image/png;base64,' + qrBase64;
    ...
    } else if (data.reason && data.reason.indexOf('already connect') !== -1) {
