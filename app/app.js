      ...
      async function pollDeviceStatus() {
        try {
          ...
          if (_lastDeviceStatus !== null && _lastDeviceStatus !== statusKey) {
            ...
            // Store the status change for the notification inbox
            const notifKey = `fonnte_${statusKey}_${todayISO()}`;
            if (!_isFonnteNotifDismissed(notifKey)) {
              _lastFonnteStatusForInbox = { status: statusKey, connected, time: new Date().toISOString(), key: notifKey };
              refreshNotifBadge();
            }
...
function dismissAllNotifs() {
  ...
  items.forEach(el => {
    ...
    if (id && id.startsWith('wa_queue_')) {
      _setDismissedQueueCount(_waQueueNotifCount);
    } else if (id && id.startsWith('fonnte_')) {
      _dismissFonnteNotif(id);
    } else if (id) {
      addDismissed(id);
    }
  });
  // Also clear WA result
  _persistWaResult(null);
  renderNotifPanel();
  updateNotifBadge(0);
}
...
async function loadNotifData() {
  ...
  return { events, waResult: _lastWaResult, queueCount: _waQueueNotifCount, fonnteStatus: _lastFonnteStatusForInbox };
}

async function refreshNotifBadge() {
  const { events, waResult, queueCount, fonnteStatus } = await loadNotifData();
  const _queueDismissed = _getDismissedQueueCount();
  const showQueue = queueCount > 0 && queueCount > _queueDismissed;
  const showFonnte = fonnteStatus && !_isFonnteNotifDismissed(fonnteStatus.key);
  updateNotifBadge(events.length + (waResult ? 1 : 0) + (showQueue ? 1 : 0) + (showFonnte ? 1 : 0));
}
...
async function renderNotifPanel() {
  ...
  const { events, waResult, queueCount, fonnteStatus } = await loadNotifData();
  const _queueDismissed2 = _getDismissedQueueCount();
  const _showQueueCard2  = queueCount > 0 && queueCount > _queueDismissed2;
  const total = events.length + (waResult ? 1 : 0) + (_showQueueCard2 ? 1 : 0) + (fonnteStatus && !_isFonnteNotifDismissed(fonnteStatus.key) ? 1 : 0);
  ...
  if (fonnteStatus && !_isFonnteNotifDismissed(fonnteStatus.key)) {
    const connected = fonnteStatus.connected;
    const color = connected ? 'var(--success)' : 'var(--error)';
    const icon  = connected ? '🟢' : '🔴';
    const title = connected ? 'واتساب متصل الآن ✅' : 'واتساب انقطع الاتصال ❌';
    const sub   = connected ? 'الجهاز جاهز للإرسال' : 'تحقق من هاتفك واتصاله بالإنترنت';
    html += `
      <div class="notif-card" style="border-right:3px solid ${color}" data-notif-id="${fonnteStatus.key}">
        <div class="notif-card-icon">${icon}</div>
        <div class="notif-card-body">
          <div class="notif-card-title">${title}</div>
          <div class="notif-card-sub">${sub}</div>
        </div>
        <button class="notif-card-dismiss" onclick="_dismissFonnteNotif('${fonnteStatus.key}');_lastFonnteStatusForInbox=null;renderNotifPanel()" title="تجاهل">✕</button>
      </div>`;
  }

  // ── WA queue pending card ──
  const _queueDismissed = _getDismissedQueueCount();
  const _showQueueCard  = queueCount > 0 && queueCount > _queueDismissed;
  if (_showQueueCard) {
    html += `
      <div class="notif-card" style="border-right:3px solid var(--warn)" data-notif-id="wa_queue_${queueCount}">
        <div class="notif-card-icon" style="cursor:pointer" onclick="closeNotifPanel();navigate('whatsapp')">💬</div>
        <div class="notif-card-body" style="cursor:pointer" onclick="closeNotifPanel();navigate('whatsapp')">
        ...
        <button class="notif-card-dismiss" onclick="dismissQueueCard()" title="تجاهل حتى وصول رسائل جديدة">✕</button>
      </div>`;
  }
  ...
  if (waResult) {
    ...
    html += `
      <div class="notif-card" style="border-right:3px solid ${color}">
        ...
        <button class="notif-card-dismiss" onclick="_persistWaResult(null);renderNotifPanel()" title="تجاهل">✕</button>
      </div>`;
  }
...
const _WA_RESULT_KEY       = 'halaqat_wa_result_v2';
const _WA_QUEUE_DISMISS_KEY = 'halaqat_wa_queue_dismiss_count';
const _FONNTE_NOTIF_KEY    = 'halaqat_fonnte_status_notifs';

let _lastWaResult = null; // {sent, failed, error, time}
let _waQueueNotifCount = 0; // live queue count for notif panel
let _lastFonnteStatusForInbox = null; // {status:'connect'|'disconnect', time}

// Restore persisted WA result from localStorage on startup
try {
  const _stored = localStorage.getItem(_WA_RESULT_KEY);
  if (_stored) _lastWaResult = JSON.parse(_stored);
} catch(e) {}

function _persistWaResult(result) {
  _lastWaResult = result;
  try {
    if (result) localStorage.setItem(_WA_RESULT_KEY, JSON.stringify(result));
    else localStorage.removeItem(_WA_RESULT_KEY);
  } catch(e) {}
}

// WA queue dismissal: store the count we dismissed at — only re-show when count exceeds it
function _getDismissedQueueCount() {
  try { return parseInt(localStorage.getItem(_WA_QUEUE_DISMISS_KEY) || '0', 10); } catch(e) { return 0; }
}
function _setDismissedQueueCount(n) {
  try { localStorage.setItem(_WA_QUEUE_DISMISS_KEY, String(n)); } catch(e) {}
}
function dismissQueueCard() {
  _setDismissedQueueCount(_waQueueNotifCount);
  renderNotifPanel();
  refreshNotifBadge();
}

// Fonnte status inbox helpers
function _getFonnteNotifDismissed() {
  try { return JSON.parse(localStorage.getItem(_FONNTE_NOTIF_KEY) || '[]'); } catch(e) { return []; }
}
function _dismissFonnteNotif(key) {
  const list = _getFonnteNotifDismissed();
  if (!list.includes(key)) { list.push(key); try { localStorage.setItem(_FONNTE_NOTIF_KEY, JSON.stringify(list)); } catch(e) {} }
}
function _isFonnteNotifDismissed(key) {
  return _getFonnteNotifDismissed().includes(key);
}
...
function showWaFailNotif(context, failCount, errorMsg) {
  // Alias for WA failure notifications — routes to summary
  showWaSummaryNotif(0, failCount || 1, errorMsg || context);
}

function showWaSummaryNotif(sent, failed, error) {
  _persistWaResult({ sent, failed, error, time: new Date().toISOString() });
  refreshNotifBadge();
