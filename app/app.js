  ...
  if (data.valid) {
    ...
    (function() {
      ...
      function showFonnteNotif(connected) {
        ...
        notif.innerHTML = '<span style="font-size:20px">' + (connected ? '🟢' : '🔴') + '</span>' +
          '<div style="flex:1"><div>' + (connected ? 'واتساب متصل الآن ✅' : 'واتساب انقطع الاتصال ❌') + '</div>' +
      ...
      async function pollDeviceStatus() {
        try {
          ...
          // Also update the device card if it's visible
          if (document.getElementById('waDeviceStatusBar') &&
              !document.getElementById('waDeviceStatusBar').classList.contains('hidden')) {
            fonnteShowDeviceCard(connected, data);
          }

          // Only notify on actual change, not first poll
...
function showWaFailNotif(context, failCount, errorMsg) {
  ...
  notif.innerHTML = '<span style="font-size:20px">❌</span>' +
  ...
  list.innerHTML = templates.map(t => `
    <div class="template-item" id="tpl-${t.id}">
      ...
      <div class="template-item-actions">
        <button class="btn-secondary" style="font-size:12px;padding:4px 10px" onclick="editTemplate('${t.id}')">✏️ تعديل</button>
        <button class="btn-secondary" style="font-size:12px;padding:4px 10px;color:var(--error)" onclick="deleteTemplate('${t.id}')">🗑</button>
      </div>
    </div>
  `).join('');
}
...
async function saveTemplate() {
  ...
  if (!name) return toast('⚠️ اسم القالب مطلوب');
  if (!body) return toast('⚠️ نص القالب مطلوب');
  const templates = [...(state.waTemplates||[])];
...
let _tplPickerTargetId = null;

function waOpenTemplatePicker(targetInputId) {
  _tplPickerTargetId = targetInputId;
  const templates = state.waTemplates || [];

  if (!document.getElementById('tplPickerModal')) {
    const modal = document.createElement('div');
    modal.id        = 'tplPickerModal';
    modal.className = 'modal-overlay';
    modal.style.cssText = 'z-index:10000';
    modal.innerHTML =
      '<div class="modal" style="max-width:480px;width:92%;max-height:80vh;display:flex;flex-direction:column">' +
        '<div class="modal-header" style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid var(--border)">' +
          '<h3 style="margin:0;font-size:16px">📋 اختر قالب رسالة</h3>' +
          '<button class="btn-icon" onclick="closeModal(\'tplPickerModal\')">✕</button>' +
        '</div>' +
        '<div id="tplPickerModalBody" style="overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:10px;flex:1"></div>' +
      '</div>';
    modal.addEventListener('click', function(e) { if (e.target === modal) modal.classList.add('hidden'); });
    document.body.appendChild(modal);

    if (!document.getElementById('tplPickerStyles')) {
      const style = document.createElement('style');
      style.id = 'tplPickerStyles';
      style.textContent =
        '.tpl-picker-card{border:1.5px solid var(--border);border-radius:12px;padding:14px 16px;cursor:pointer;transition:border-color .15s,box-shadow .15s,background .15s;background:var(--surface)}' +
        '.tpl-picker-card:hover{border-color:var(--primary);box-shadow:0 0 0 3px #dbeafe;background:#eff6ff}' +
        '.tpl-picker-name{font-size:14px;font-weight:700;color:var(--primary);margin-bottom:6px}' +
        '.tpl-picker-preview{font-size:12px;color:var(--text2);line-height:1.6;white-space:pre-wrap}';
      document.head.appendChild(style);
    }
  }

  const body = document.getElementById('tplPickerModalBody');
  if (!templates.length) {
    body.innerHTML =
      '<div style="text-align:center;padding:32px 16px;color:var(--text2)">' +
        '<div style="font-size:36px;margin-bottom:12px">📋</div>' +
        '<div style="font-size:15px;font-weight:600;margin-bottom:6px">لا توجد قوالب بعد</div>' +
        '<div style="font-size:13px">أضف قوالب من صفحة الإعدادات</div>' +
        '<button class="btn-secondary" style="margin-top:16px" onclick="closeModal(\'tplPickerModal\');navigate(\'settings\')">⚙️ الإعدادات</button>' +
      '</div>';
  } else {
    body.innerHTML = templates.map(function(t) {
      return '<div class="tpl-picker-card" onclick="waApplyTemplate(\'' + targetInputId + '\',\'' + t.id + '\')">' +
        '<div class="tpl-picker-name">' + t.name + '</div>' +
        '<div class="tpl-picker-preview">' + t.body.slice(0,120) + (t.body.length > 120 ? '…' : '') + '</div>' +
      '</div>';
    }).join('');
  }
  document.getElementById('tplPickerModal').classList.remove('hidden');
}

function waApplyTemplate(targetInputId, templateId) {
  const t = (state.waTemplates || []).find(function(x) { return x.id === templateId; });
  if (!t) return;
...
function waInjectTemplatePicker(targetInputId) {
  const templates = state.waTemplates || [];
  const pickerId  = 'tplPickerWrap_' + targetInputId;
  document.getElementById(pickerId)?.remove();
  const input = document.getElementById(targetInputId);
  if (!input || !templates.length) return;
  const wrapper = document.createElement('div');
  wrapper.id = pickerId;
  wrapper.style.cssText = 'margin-bottom:8px;';
  wrapper.innerHTML =
    '<button type="button" ' +
    'style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;background:var(--surface);color:var(--text1);cursor:pointer;font-family:inherit" ' +
    'onclick="waOpenTemplatePicker(\'' + targetInputId + '\')">' +
    '📋 <span>استخدام قالب</span></button>';
...
function saveGClientId() {
  const val = document.getElementById('settGClientId')?.value.trim();
  const st  = document.getElementById('gClientIdStatus');
  if (!val) { st.style.color='var(--error)'; st.textContent='⚠️ أدخل Client ID أولاً'; return; }
...
async function driveBackup() {
  ...
  const btn   = document.getElementById('dashDriveBtn');
  const label = document.getElementById('dashDriveBtnLabel');

  if (!clientId) {
    toast('⚠️ أدخل Google Client ID في الإعدادات أولاً');
    navigate('settings');
    return;
  }

  // Step 1 — get backup blob from server
  label.textContent = '⏳ جارٍ التحضير…';
  let blob;
  try {
    const res = await fetch('/api/sync/export');
    if (!res.ok) throw new Error('فشل تحضير النسخة');
    blob = await res.blob();
  } catch(e) {
    label.textContent = 'نسخ Drive';
    toast('❌ فشل تحضير النسخة الاحتياطية');
    return;
  }

  // Step 2 — get OAuth token via PKCE popup
  label.textContent = '🔐 تسجيل الدخول…';
  let token;
  try {
    token = await _gdriveGetToken(clientId);
  } catch(e) {
    label.textContent = 'نسخ Drive';
    toast('❌ ' + (e.message || 'فشل تسجيل الدخول بـ Google'));
    return;
  }

  // Step 3 — upload to Drive
  label.textContent = '☁️ جارٍ الرفع…';
  try {
    const date = new Date().toISOString().split('T')[0];
    const fileName = `backup-halaqa-${date}.json`;
    await _gdriveUpload(token, blob, fileName);
    label.textContent = 'نسخ Drive';
    localStorage.setItem('last_backup_ts', Date.now().toString());
    refreshNotifBadge();
    toast('✅ تم رفع النسخة الاحتياطية إلى Google Drive');
  } catch(e) {
    label.textContent = 'نسخ Drive';
    toast('❌ فشل الرفع: ' + (e.message || 'خطأ غير معروف'));
  }
}

function _gdriveGetToken(clientId) {
  return new Promise((resolve, reject) => {
    ...
    // PKCE OAuth2 implicit flow (simplest for SPAs)
    ...
    if (!popup) return reject(new Error('تعذّر فتح نافذة تسجيل الدخول — السماح بالنوافذ المنبثقة'));
    ...
    const poll = setInterval(() => {
      try {
        if (popup.closed) {
          clearInterval(poll);
          return reject(new Error('تم إغلاق نافذة تسجيل الدخول'));
        }
        ...
        if (hash && hash.includes('access_token')) {
          ...
          localStorage.setItem('gdrive_token', JSON.stringify({
            token, expires: Date.now() + (expiresIn - 60) * 1000
          }));
          resolve(token);
        }
    ...
    // Timeout after 3 min
    setTimeout(() => { clearInterval(poll); if (!popup.closed) popup.close(); reject(new Error('انتهت مهلة تسجيل الدخول')); }, 180000);
  });
}

async function _gdriveUpload(token, blob, fileName) {
  // Multipart upload to Google Drive
  const metadata = JSON.stringify({ name: fileName, mimeType: 'application/json' });
  const boundary = '-------halaqa_boundary';
  const body = [
  ...
  const total = new Uint8Array(part1.byteLength + part2.byteLength + blobBuf.byteLength + part3.byteLength);
  let offset = 0;
  [part1, part2, new Uint8Array(blobBuf), part3].forEach(b => { total.set(b, offset); offset += b.byteLength; });
...
async function initSettings() {
  renderTemplateList();
  const gId = localStorage.getItem('gdrive_client_id') || '';
  const el = document.getElementById('settGClientId');
  if (el) el.value = gId;
...
async function confirmBulkImport() {
  ...
  if (!classId) return toast('⚠️ يرجى اختيار الحلقة الافتراضية قبل الاستيراد');
...
async function _doImport(students, classId, forceAdd) {
  ...
  if (res?.needsConfirm && res.duplicates?.length) {
    if (btn) { btn.disabled = false; btn.textContent = '✅ استيراد الطلاب'; }
    ...
    result.innerHTML = `
      <div style="border:1.5px solid var(--warn);border-radius:12px;padding:16px;background:var(--warn-l)">
        <div style="font-size:15px;font-weight:700;color:var(--warn);margin-bottom:8px">⚠️ تم اكتشاف ${res.duplicates.length} طالب مكرر</div>
        ...
        ${res.added > 0 ? `<div style="margin-bottom:12px;font-size:13px;color:var(--success)">✅ تمت إضافة ${res.added} طالب جديد بنجاح</div>` : ''}
...
function waShowTab(tab) {
  ...
  if (tab === 'send') {
    setTimeout(() => {
      waInjectTemplatePicker('waCustomMsg');
      waInjectTemplatePicker('waGroupChatMsg');
    }, 60);
  }
}
...
function waUpdateSendBtn() {
  ...
  if (delBtn) {
    ...
    delBtn.textContent = count > 0 ? `🗑 حذف ${count}` : '🗑 حذف المحدد';
  }
}
...
async function waDeleteSelected() {
  ...
  toast(`🗑 تم حذف ${deleted} عنصر من قائمة الانتظار`);
}
...
function waRenderLog() {
  ...
  el.innerHTML = combined.map(entry => {
    ...
    if (entry.status === 'sent') {
      ...
      statusClass = 'wa-log-partial'; dotClass = 'wa-dot-partial'; statusLabel = '⚠️ جزئي — ' + (entry._failCount||'?') + '/' + (entry._totalCount||'?') + ' فشلت';
    } else if (entry.status === 'waiting') {
      statusClass = 'wa-log-waiting'; dotClass = 'wa-dot-waiting'; statusLabel = '⏳ في انتظار الإرسال';
    } else if (entry.status === 'not_scheduled') {
      statusClass = 'wa-log-failed';  dotClass = 'wa-dot-failed';  statusLabel = '❌ لم يُجدَّل';
      ...
      <div class="wa-log-item ${statusClass}${isSelected?' wa-log-item-selected':''}" data-log-id="${entry.id}">
        ...
        <div class="wa-log-item-left">
          ...
          <div class="wa-log-content">
            ...
            ${(entry.status === 'failed' || entry.status === 'partial' || entry.status === 'not_scheduled') && entry.error ? `<div class="wa-log-error">⚠️ ${entry.error}</div>` : ''}
...
function calSelectType(type){
  ...
  if (isMsg) {
    _calWaPopulateGroupSelect();
    setTimeout(() => waInjectTemplatePicker('calWaMessage'), 60);
  }
}
...
async function saveCalEvent(){
  ...
  if(id){
    ...
    if (payload.type === 'message' && payload.waTargets?.length) {
      ...
      else if (sr)   { toast(`⚠️ تم تحديث الحدث لكن فشل إعادة الجدولة: ${sr.error||'تحقق من الـ Token'}`); showWaFailNotif('رسالة مجدولة', 1, sr.error || 'تحقق من الـ Token'); }
    ...
    if((dailyRepeat || useSpecific) && r?.ids?.length>1) {
      ...
      else if(failed>0) { toast(`⚠️ تم حفظ الأحداث لكن فشل الجدولة على Fonnte`); showWaFailNotif('رسائل مجدولة', failed, 'فشل الجدولة على Fonnte'); }
      ...
      else if(sr)   { toast(`⚠️ تم حفظ الحدث لكن فشل الجدولة على Fonnte: ${sr.error||'تحقق من الـ Token'}`); showWaFailNotif('رسالة مجدولة', 1, sr.error || 'تحقق من الـ Token'); }
...
function woToggleMsg() {
  ...
  if (on) setTimeout(() => waInjectTemplatePicker('woMessage'), 60);
}
...
async function fonnteLoadQR() {
  ...
  try {
    ...
    if (data.status === true && data.base64) {
...
function fonnteShowDeviceCard(connected, dev) {
  ...
  if (iconEl)  iconEl.textContent  = connected ? 'متصل' : 'غير متصل';
