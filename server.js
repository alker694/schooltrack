...
// ── Fonnte generic proxy helper ───────────────────────
function fonnteGet(path, token) {
  return new Promise(resolve => {
    const opts = {
      hostname: 'api.fonnte.com',
      path,
    ...
    const req2 = https.request(opts, resp => {
      ...
      resp.on('end', () => {
        ...
        console.log(`[fonnte] GET ${path} → HTTP ${resp.statusCode} | CT: ${ct} | body: ${raw.slice(0,300)}`);
        if (ct.includes('image')) {
          resolve({ status: true, data: buf.toString('base64'), mime: ct.split(';')[0].trim() });
        } else {
          try {
            const j = JSON.parse(raw);
            resolve(j);
          } catch(e) {
            resolve({ status: false, reason: 'parse-error', raw: raw.slice(0, 300) });
          }
        }
      });
    });
    req2.on('error', e => { console.error(`[fonnte] GET ${path} error:`, e.message); resolve({ status: false, reason: e.message }); });
    req2.end();
  });
}

// ── Fonnte Proxy: QR Code ─────────────────────────────
app.get('/api/fonnte/qr', async (req, res) => {
  const { settings } = readDB();
  const token = settings.whatsappApiKey;
  if (!token) return res.json({ status: false, reason: 'Fonnte Token غير مُعيَّن في الإعدادات' });
  try {
    const data = await fonnteGet('/qr', token);
    // Normalize: Fonnte sometimes wraps the QR URL inside data.url or returns base64 directly
    if (data.status === true && (data.url || data.data)) {
      // Fonnte returns base64 image in `url` field (despite the name)
      const b64 = data.url || data.data;
      res.json({ status: true, base64: b64, raw: data });
    } else {
      res.json({ status: false, reason: data.reason || data.detail || data.message || 'unknown', raw: data });
    }
  } catch(e) {
    res.json({ status: false, reason: e.message });
  }
});

// ── Fonnte Proxy: Device Status ────────────────────────
app.get('/api/fonnte/device', async (req, res) => {
  const { settings } = readDB();
  const token = settings.whatsappApiKey;
  if (!token) return res.json({ status: false, reason: 'Fonnte Token غير مُعيَّن في الإعدادات' });
  try {
    const data = await fonnteGet('/device', token);
    res.json(data);
  } catch(e) {
    res.json({ status: false, reason: e.message });
  }
...
// ── Fonnte Proxy: Debug (raw response dump) ────────────
// Visit /api/fonnte/debug in browser to see exact Fonnte responses
app.get('/api/fonnte/debug', async (req, res) => {
  const { settings } = readDB();
  const token = settings.whatsappApiKey;
  if (!token) return res.json({ error: 'no token saved' });
  const [qr, device] = await Promise.all([
    fonnteGet('/qr', token),
    fonnteGet('/device', token),
  ]);
  res.json({ token_preview: token.slice(0,6)+'…', qr, device });
});
...
// ── SSE: real-time queue count push ─────────────────────────────
const _ssQueueClients = new Set();

function broadcastQueueCount() {
  if (_ssQueueClients.size === 0) return;
  try {
    const db = readDB();
    const sentKeys = new Set(
      (db.waLog||[]).filter(l=>l.status==='sent'&&l.studentId&&l.date)
                    .map(l=>`${l.studentId}__${l.date}`)
    );
    const dismissedKeys = new Set(
      (db.waDismissed||[]).filter(d=>d.studentId&&d.date)
                          .map(d=>`${d.studentId}__${d.date}`)
    );
    const cutoff = new Date(Date.now()-7*24*60*60*1000).toISOString().split('T')[0];
    const count = (db.attendance||[])
      .filter(a => a.status==='Absent' && a.date>=cutoff)
      .filter(a => { const k=`${a.studentId}__${a.date}`; return !sentKeys.has(k)&&!dismissedKeys.has(k); })
      .length;
    const msg = `data: ${count}\n\n`;
    _ssQueueClients.forEach(res => { try { res.write(msg); } catch(e) { _ssQueueClients.delete(res); } });
  } catch(e) { console.error('[sse] broadcastQueueCount error:', e.message); }
}
...
app.get('/api/whatsapp/queue/stream', (req, res) => {
  ...
  res.setHeader('X-Accel-Buffering', 'no'); // nginx/render proxy fix
  res.flushHeaders();
  _ssQueueClients.add(res);
  // Send current count immediately on connect
  broadcastQueueCount();
  req.on('close', () => { _ssQueueClients.delete(res); });
});
