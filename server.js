...
if (fs.existsSync(APP_DIR)) {
  ...
  // Serve modules/ subfolder if it exists inside app/
  const APP_MODULES = path.join(APP_DIR, 'modules');
  if (fs.existsSync(APP_MODULES)) app.use('/modules', express.static(APP_MODULES));
}
app.use(express.static(ROOT_DIR));
// Serve modules/ from root dir too (fallback)
const ROOT_MODULES = path.join(ROOT_DIR, 'modules');
if (fs.existsSync(ROOT_MODULES)) app.use('/modules', express.static(ROOT_MODULES));
